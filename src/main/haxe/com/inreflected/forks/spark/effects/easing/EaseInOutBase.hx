...
class EaseInOutBase implements IEaser
{
	...
	@:meta(Inspectable(minValue="0.0",maxValue="1.0"))
	...
	{
		var easeOutFraction : Float = 1 - easeInFraction;
		if (fraction <= easeInFraction && easeInFraction > 0) 
			return easeInFraction * easeIn(fraction / easeInFraction)
		else return easeInFraction + easeOutFraction * easeOut((fraction - easeInFraction) / easeOutFraction);
	}
     ...
     *  @productversion Flex 4
     */
	function easeIn(fraction : Float) : Float
	{
		return fraction;
	}
     ...
     *  @productversion Flex 4
     */
	function easeOut(fraction : Float) : Float
	{
		return fraction;
	}
